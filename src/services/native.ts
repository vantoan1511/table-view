...
export class NativeService {
   ...
   */
  static os = {
    async showSaveDialog(title: string, options: { filters: Array<{ name: string; extensions: string[] }> }): Promise<string | null> {
      if (!window.NL_PORT) return null
    ...
    async showMessageBox(title: string, content: string, icon: 'INFO' | 'WARNING' | 'ERROR' | 'QUESTION' = 'INFO'): Promise<void> {
      if (!window.NL_PORT) return
