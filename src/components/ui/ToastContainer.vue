...
function getPositionClass(pos: string) {
  switch (pos) {
    ...
    default: return 'bottom-4 right-4 items-end flex-col-reverse'
  }
}
</script>
...
<template>
  <div class="fixed inset-0 pointer-events-none z-9999 overflow-hidden">
    <TransitionGroup v-for="(toasts, pos) in groupedToasts" :key="pos" name="toast" tag="div"
      class="absolute flex gap-3 pointer-events-none" :class="getPositionClass(String(pos))">
      <ToastMessage v-for="toast in toasts" :key="toast.id" :toast="toast" @close="toastStore.removeToast(toast.id!)" />
    </TransitionGroup>
  </div>
</template>
