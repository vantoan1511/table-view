import { ref } from 'vue'
import type { GridColumn, GridRow, Connection } from '@/types'
...
export function useTableData(connectionsStore: any) {
  ...
  const resolveConnection = (connectionId?: string) =>
    connectionsStore.connections.find((conn: Connection) => conn.id === (connectionId || connectionsStore.activeConnectionId)) ?? null
  ...
  const loadTable = async (tableName: string, isLoading: any, connectionId?: string, schemaName?: string, dbName?: string) => {
    ...
    try {
      const payload = await BridgeService.request('dbBridge.fetchTableData', 'dbBridge.fetchTableDataResult', {
