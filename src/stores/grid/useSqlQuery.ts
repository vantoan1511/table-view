...
export function useSqlQuery(connectionsStore: any) {
  ...
  const runQuery = async (sql: string, isLoading: any, limit?: number, connectionId?: string, dbName?: string) => {
    ...
    try {
      const payload = await BridgeService.request('dbBridge.executeQuery', 'dbBridge.executeQueryResult', {
