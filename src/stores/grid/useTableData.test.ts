...
describe('useTableData', () => {
  ...
  describe('loadTable', () => {
    it('calls BridgeService.request with correct parameters', async () => {
      ...
      const mockPayload = {
        rows: [{ id: 1, name: 'Test' }],
        fields: [{ name: 'id', dataTypeID: 23, isPrimaryKey: true }, { name: 'name', dataTypeID: 1043 }],
        totalCount: 1,
        executionTime: 10
      }
      ...
      expect(tableData.columns.value[0]).toEqual({ name: 'id', dataType: '23', isPrimaryKey: true })
