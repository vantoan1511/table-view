...
description: Coding patterns from table-view repository
version: 1.0.0
...
└── views/          # Vue views
extensions/
└── db-bridge/      # Rust backend logic
```

## Workflow Mandates

### Validation
- After modifying the Rust extension, always perform a `cargo build` to ensure the binary compiles correctly before testing with NeutralinoJS.

### Logging
- Maintain a centralized log file (`.log`) in the application root directory, capturing both backend (Rust) and frontend (UI) diagnostic information for unified troubleshooting.

### Pathing
- Use Windows-compatible backslash paths (`\\`) in `neutralino.config.json` for extension commands.
